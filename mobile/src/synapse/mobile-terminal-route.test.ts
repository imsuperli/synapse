...
describe('Synapse Mobile terminal route wiring', () => {
  it('loads terminal history before subscribing to live output without resizing the desktop PTY', () => {
    ...
    expect(routeSource).toContain('const screenSnapshotData = history.screenSnapshot?.alternate')
    expect(routeSource).toContain('history.screenSnapshot.windowId === windowId')
    ...
    expect(routeSource).toContain('screenSnapshotDataRef.current = screenSnapshotData')
