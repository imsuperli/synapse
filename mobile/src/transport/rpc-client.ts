...
export function connect(
  ...
  const streamListenersBySubscriptionId = new Map<string, StreamingListener>()
      ...
      if (timeoutMs !== undefined) {
        ...
        waiter.timeout = setTimeout(
          () => {
            ...
            reject(new Error('Timed out while connecting to the remote Synapse desktop.'))
          },
  ...
  function openConnection() {
    ...
    async function handleSocketMessage(rawData: unknown) {
      ...
      if (isSynapseStreamEvent(response)) {
        const listener = streamListenersBySubscriptionId.get(response.subscriptionId)
        if (listener) {
      ...
      if (response.ok) {
        ...
        if (result && typeof result.subscriptionId === 'string') {
          const stream = streamListeners.get(response.id)
          if (stream) {
            stream.subscriptionId = result.subscriptionId
  ...
  function removeStreamListener(id: string): void {
    ...
    if (stream?.subscriptionId) {
      streamListenersBySubscriptionId.delete(stream.subscriptionId)
    }
...
function isSynapseStreamEvent(
  value: unknown
): value is { type: 'event'; subscriptionId: string; payload: unknown } {
  return (
    !!value &&
    typeof value === 'object' &&
