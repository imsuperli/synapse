...
React Native companion app for Synapse. Monitor worktrees, view terminal output, and send commands from your phone.

Local development uses two processes:

- Synapse desktop/Electron from the repo root. This hosts the mobile WebSocket RPC server on port `6768`.
...
1. Open GitHub > Actions > Test Build.
2. Click `Run workflow`.
3. Choose `android` to build only the APK, or `all` to build desktop installers and the APK together.
4. When the run finishes, download the `android-apk-test` artifact.
5. Install `synapse-mobile-android.apk` on your Android phone.

The dedicated GitHub > Actions > Mobile workflow can also build the APK manually; its artifact is named `synapse-mobile-android-apk`.
...
```bash
synapse snapshot --json
synapse click --element @e3 --json
synapse fill --element @e1 --value "ls" --json
synapse screenshot --json
```
...
## Connecting to Real Synapse

1. Start Synapse desktop with WebSocket transport enabled
2. In Synapse, go to Settings > Mobile and scan the QR code with this app
