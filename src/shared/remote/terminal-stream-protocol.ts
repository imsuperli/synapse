...
export function terminalStreamByteLength(value: string): number {
  return encodeTerminalStreamText(value).byteLength;
}

export function terminalStreamByteLengthExceeds(value: string, maxBytes: number): boolean {
  let bytes = 0;
  for (const part of value) {
    bytes += terminalStreamByteLength(part);
    if (bytes > maxBytes) {
      return true;
    }
  }
...
export function* iterateTerminalStreamTextPayloads(
  ...
  for (const part of value) {
    ...
    if (chunk && chunkBytes + partBytes > maxChunkBytes) {
      yield encodeTerminalStreamText(chunk);
      chunk = '';
      chunkBytes = 0;
    }
    chunk += part;
